...
# Writes the Alabaster GNOME Terminal theme colors and configurations as dconf key-value pairs to the target profile.
#
...
apply() {
  ...
  log 4 "Applied Alabaster color palette"

  _write background-color "'$primary_white'"
  _write foreground-color "'$primary_black'"
  _write use-transparent-background "false"
  log 4 "Applied background- and foreground colors"

  _write bold-color "'$primary_black'"
  ...
  _write cursor-foreground-color "'$primary_white'"
  _write cursor-background-color "'$primary_black'"
  ...
  _write "$ALABASTER_GNOME_TERMINAL_VERSION_DCONF_KEY" "'$ALABASTER_GNOME_TERMINAL_VERSION'"
  log 4 "Set Alabaster GNOME Terminal version key of the '$profile_name' profile"

  log 3 "Applied theme colors and configurations"
}

# Applies the Alabaster GNOME Terminal theme with version comparison.
#
...
apply_version_compared() {
  ...
  version=$1
  case "$(
    vercomp "$ALABASTER_GNOME_TERMINAL_VERSION" "$version"
    echo $?
  )" in
    0)
      log 3 "Reinstalling Alabaster GNOME Terminal since the version equaled the version of the '$profile_name' profile!"
      apply
      log 2 "Reinstalled Alabaster GNOME Terminal version $ALABASTER_GNOME_TERMINAL_VERSION for the '$profile_name' profile!"
      exit 0
      ;;
    1)
      log 4 "The script version is newer than the currently installed theme ($ALABASTER_GNOME_TERMINAL_VERSION > $version)"
      apply
      log 2 "The '$profile_name' profile has been updated successfully from version $version to $ALABASTER_GNOME_TERMINAL_VERSION"
      exit 0
      ;;
    2)
      log 1 "The detected Alabaster GNOME Terminal version $version of the '$profile_name' profile is greater than the version that will be applied!"
      printf "${_ctb}> [?]${_ct} Override current profile version %s with script version %s? (${_ctb}y${_cr}/${_ctb}n${_cr})" "$version" "$ALABASTER_GNOME_TERMINAL_VERSION"
      read -r -n 1 -s confirmation
      echo
      case $confirmation in
        [Yy]*)
          apply
          log 2 "Alabaster GNOME Terminal version $ALABASTER_GNOME_TERMINAL_VERSION has been successfully applied!"
          exit 0
          ;;
        [Nn]*)
...
cleanup() {
  ...
  unset -v ALABASTER_GNOME_TERMINAL_SCRIPT_OPTS ALABASTER_GNOME_TERMINAL_VERSION ALABASTER_GNOME_TERMINAL_VERSION_DCONF_KEY ALABASTER_PROFILE_VISIBLE_NAME log_level DEPENDENCIES DCONF_PROFILE_BASE_PATH GSETTINGS_PROFILELIST_PATH gnome_terminal_version profile_name profile_uuid
...
clone_default_profile() {
  ...
  dconf write "$DCONF_PROFILE_BASE_PATH"/:"$profile_uuid"/visible-name "'$ALABASTER_PROFILE_VISIBLE_NAME'"
  append_profile_uuid_to_list "$profile_uuid"
  log 3 "Cloned the default profile '$uuid' with new UUID '$profile_uuid'"
}

# Gets the current Alabaster GNOME Terminal version.
#
# @globread ALABASTER_GNOME_TERMINAL_VERSION_DCONF_KEY DCONF_PROFILE_BASE_PATH
...
get_current_version() {
  ...
  uuid=$1
  version=
  printf "$(dconf read $DCONF_PROFILE_BASE_PATH/:"$uuid"/$ALABASTER_GNOME_TERMINAL_VERSION_DCONF_KEY | tr -d "'")"
  return 0
}
...
ALABASTER_GNOME_TERMINAL_SCRIPT_OPTS=$(getopt -o hl:p: --long help,loglevel:,profile: -n 'install.sh' -- "$@")
ALABASTER_GNOME_TERMINAL_VERSION=0.1.0
ALABASTER_GNOME_TERMINAL_VERSION_DCONF_KEY=alabaster-gnome-terminal-version
ALABASTER_PROFILE_VISIBLE_NAME="Alabaster"
log_level=2
...
eval set -- "$ALABASTER_GNOME_TERMINAL_SCRIPT_OPTS"
while true; do
  case "$1" in
    --loglevel=*)
      log_level=${1#*=}
...
if validate_dependencies DEPENDENCIES[@]; then
  ...
  if [[ -n $profile_name ]]; then
    ...
    if [[ -n $profile_uuid ]]; then
      ...
      if [[ -n $curr_ver ]]; then
        ...
        apply
        log 2 "Alabaster GNOME Terminal version $ALABASTER_GNOME_TERMINAL_VERSION has been successfully applied to the '$profile_name' profile"
        exit 0
      fi
    else
  ...
  profile_uuid="$(get_profile_uuid_by_name $ALABASTER_PROFILE_VISIBLE_NAME)"
  if [[ -n $profile_uuid ]]; then
    profile_name="$ALABASTER_PROFILE_VISIBLE_NAME"
    log 4 "Found already existing '$ALABASTER_PROFILE_VISIBLE_NAME' profile with UUID '$profile_uuid'"
    log "Updating already existing '$ALABASTER_PROFILE_VISIBLE_NAME' profile"
    apply_version_compared "$(get_current_version $profile_uuid)"
  fi

  clone_default_profile
  apply
  log 2 "Alabaster GNOME Terminal version $ALABASTER_GNOME_TERMINAL_VERSION has been successfully applied to the newly created '$ALABASTER_PROFILE_VISIBLE_NAME' profile"
  exit 0
else
